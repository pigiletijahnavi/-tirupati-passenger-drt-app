...
st.markdown(
    """
    <h1 style='text-align:center;'> AI-Powered Demand Responsive Transit (DRT)</h1>
    <h4 style='text-align:center;'>Smart Pilgrim Transport Optimization – Tirupati</h4>
    """,
...
if run or st.session_state.df is not None:
    ...
    if centers:
        nearest = min(centers, key=lambda c: geodesic((user_lat,user_lon),(c[0],c[1])).km)
        dist = geodesic((user_lat,user_lon), (nearest[0], nearest[1])).km
        st.success(f"Nearest Stop: {nearest} | Distance: {dist:.2f} km")
